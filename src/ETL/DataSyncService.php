...
class DataSyncService
{
    ...
    {
        // Приводим всё к int и убираем дубли
        $userIds = array_unique(array_map('intval', $userIds));

        if (empty($userIds)) {
            echo "Нет userIds для выборки\n";
            return [];
        }

        // Просто безопасно собираем строку вручную
        $inClause = implode(',', $userIds);

        $sql = "SELECT * FROM users WHERE id IN ($inClause)";
        $users = $this->source->getAll($sql);

        echo "Запрашивали " . count($userIds) . " userIds\n";
        echo "Вернулось пользователей: " . count($users) . "\n";

        // Для отладки — кто не нашёлся
        $foundIds = array_column($users, 'id');
        $missing = array_diff($userIds, $foundIds);
        if ($missing) {
            echo "Не найдены пользователи с id: " . implode(', ', $missing) . "\n";
        }
