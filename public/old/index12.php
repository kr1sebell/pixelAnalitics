...
function fetchMetrics($analytics,$dimension,DateTime $start,DateTime $end): array {
    return $analytics->getAll(
        'SELECT dimension_value,
                ...
                ROUND(SUM(total_revenue)/NULLIF(SUM(total_orders),0),2) AS avg_receipt,
...
function fetchTotals($analytics, DateTime $start, DateTime $end): array {
    return $analytics->getRow(
        'SELECT 
            ...
            SUM(total_revenue) AS total_revenue,
            SUM(total_customers) AS total_customers,
            IFNULL(ROUND(SUM(total_revenue)/NULLIF(SUM(total_orders),0),2),0) AS avg_receipt
         FROM analytics_dimension_metrics
                ...
                <div class="row">
                    <?php foreach(array_chunk($block['current'],2) as $pair): ?>
                        <?php foreach($pair as $m):
                            ...
                            $prev=$prev[0]??['total_revenue'=>0,'total_orders'=>0,'total_customers'=>0,'avg_receipt'=>0];
                            ?>
