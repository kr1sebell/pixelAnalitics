    ...
    <?php foreach($allMetrics as $dim=>$block): ?>
        <div class="card mb-4 shadow-sm">
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-6"><canvas id="chart_<?=$dim?>"></canvas></div>
                    <div class="col-md-6">
                        <table class="table table-sm table-striped">
                            <thead><tr>
                                <th>Значение</th><th>Выручка</th><th>Заказы</th><th>Клиенты</th><th>Топ товары</th>
                            </tr></thead>
                            <tbody>
                            <?php foreach($block['metrics'] as $m): ?>
                                <tr>
                                    <td><?=htmlspecialchars(formatDimensionValue($dim,$m['dimension_value']))?></td>
                                    ...
                                    <td>
                                        <?php if(!empty($block['products'][$m['dimension_value']])): ?>
                                            <?php foreach($block['products'][$m['dimension_value']] as $p): ?>
                                                <div><?=htmlspecialchars($p['title'])?> (<?=$p['quantity']?>)</div>
                                            <?php endforeach; ?>
