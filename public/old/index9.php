    ...
    <?php foreach($allMetrics as $dim=>$block): ?>
        <div class="card mb-4 shadow-sm">
            <div class="card-body">
                ...
                <p class="text-muted">
                    Текущий период: <?=$start->format('d.m.Y')?> — <?=$end->format('d.m.Y')?> |
                ...
                <table class="table table-sm align-middle">
                    ...
                    <?php foreach($block['current'] as $m):
                        ...
                        ?>
                        <tr>
                            <td><strong><?=htmlspecialchars(formatDimensionValue($dim,$val))?></strong></td>
                            <td>
    ...
    new Chart(document.getElementById("chart_<?=$dim?>"),{
        ...
        data:{
            ...
            datasets:[{
                ...
                data:<?=json_encode(array_map(function($m)use($block){
                    $val=$m['dimension_value'];
                    $prev=array_values(array_filter($block['previous'],fn($x)=>$x['dimension_value']===$val));
                    return $prev[0]['total_revenue']??0;
                },$block['current']))?>,
